...
func (runner *runner) GetInterfaces() ([]Ipv4Interface, error) {
	...
	for i, inter := range interfaces {
		...
		if val, ok := indexMap[name]; ok {			
			...
			glog.Infof("Copying index value %d", val)
		} else {
...
func (runner *runner) getNetworkInterfaceParameters() (map[string]int, error) {
	...
	for _, line := range outputLines {
		...
		glog.Infof("%v %v %v %v %v", splitLine[0], splitLine[1], splitLine[2], splitLine[3], splitLine[4])
		
		if idx, err := strconv.Atoi(splitLine[0]); err == nil {
			indexMap[name] = idx
			glog.Infof("Matched interface name %v with index %v", name, idx)
		}
