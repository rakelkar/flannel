...
func main() {
	...
	// Go routines spawned from main.go coordinate using a WaitGroup. This provides
	// a mechanism to allow the shutdownHandler goroutine to block until all the
	// goroutines return. If those goroutines spawn other goroutines then they are
	// responsible for blocking and returning only when cancel() is called.
...
func shutdownHandler(ctx context.Context, sigs chan os.Signal, cancel context.CancelFunc) {
	// Wait for the context to be Done or for the signal to come in before shutting down.
	select {
	case <-ctx.Done():
		log.Info("Stopping shutdownHandler...")
	case <-sigs:
		// Call cancel on the context to shut everything down
		cancel()
